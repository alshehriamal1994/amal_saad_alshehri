...
# ===================== HF seq-class scorer (patched for BGE / PGE, max_length<=512) =====================
...
class HFSeqClsScorer:
    ...
    def __init__(self, model_name: str, device: str = None, max_length: int = 512):
        self.device = device or ("cuda" if torch.cuda.is_available() else "cpu")
        # BGE rerankers are XLM-R with max_position_embeddings=514,
        # so we hard-cap at 512 tokens for safety.
        if max_length > 8192:
            max_length = 8192
        self.max_length = max_length
        print(f"Loading HF seq-class model {model_name} on {self.device} (max_length={self.max_length})")
        self.tokenizer = AutoTokenizer.from_pretrained(model_name)
        # Make sure tokenizer itself also clamps to this
